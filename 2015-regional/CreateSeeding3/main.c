...
void score_our_cubes() {
	...
	operate_winch(WINCH_DROP_POSITION);
	msleep(1000);
	create_spin_degrees(RIGHT,7,20);
	...
	create_spin_degrees(LEFT, 38, 20); // this moves somewhat erratically.  Why?
	create_drive_distance(FORWARDS, 130, 30);
	operate_winch(WINCH_DROP_POSITION);
	
	// Go forward until the front hits the PVC.  Square up.
	create_drive_distance(FORWARDS, 72, 20); //was 75
	...
	close_claw();
	create_drive_distance(FORWARDS, 48, 15);
	create_spin_degrees(LEFT, 90, 20);
	create_drive_distance(FORWARDS, 8, 10); 
	create_drive_distance(FORWARDS, 5, 10);
	create_drive_distance(BACKWARDS, 9, 20);
	create_spin_degrees(RIGHT, 90, 20);
	create_drive_distance(FORWARDS, 14, 20);
	create_drive_distance(FORWARDS, 5, 10);
	...
	//   -- Grab the cubes.
	operate_winch(WINCH_MIDDLE_POSITION);
	create_spin_degrees(RIGHT, 24, 10);
	open_claw();
	operate_winch(WINCH_GROUND_POSITION);
	close_claw();
	...
	//   -- Open the claw.
	operate_winch(WINCH_DROP_POSITION);
	create_spin_degrees (LEFT, 7, 10);
	create_drive_until_event(BACKWARDS, 10, BUMP); //create_drive_distance(BACKWARDS, 43, 20);
	create_spin_degrees(RIGHT, 42, 20);
	create_drive_distance(FORWARDS, 61, 20);
	create_spin_degrees(LEFT, 21, 20);
	open_claw();
