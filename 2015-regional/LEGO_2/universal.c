...
void adjust_drive(int direction, int amount) {
	...
	while (TRUE) {
		...
		if (choice == A_BUTTON) {
			...
			lego_drive_distance(FORWARDS, BIG_DRIVE_ADJUST, 10);
			adjustment += BIG_DRIVE_ADJUST;
		} else if (choice == C_BUTTON) {
			lego_drive_distance(BACKWARDS, BIG_DRIVE_ADJUST, 10);
			adjustment -= BIG_DRIVE_ADJUST;
		} else if (choice == Y_BUTTON) {
			lego_drive_distance(FORWARDS, SMALL_DRIVE_ADJUST, 10);
			adjustment += SMALL_DRIVE_ADJUST;
		} else if (choice == Z_BUTTON) {
			lego_drive_distance(BACKWARDS, SMALL_DRIVE_ADJUST, 10);
			adjustment -= SMALL_DRIVE_ADJUST;
		}
	}
...
void adjust_spin(int direction, int amount) {
	...
	while (TRUE) {
		...
		if (choice == A_BUTTON) {
			...
			lego_spin_degrees(RIGHT, BIG_SPIN_ADJUST, 10);
			adjustment += BIG_DRIVE_ADJUST;
		} else if (choice == C_BUTTON) {
			lego_spin_degrees(LEFT, BIG_SPIN_ADJUST, 10);
			adjustment -= BIG_DRIVE_ADJUST;
		} else if (choice == Y_BUTTON) {
			lego_spin_degrees(RIGHT, SMALL_SPIN_ADJUST, 10);
			adjustment += SMALL_DRIVE_ADJUST;
		} else if (choice == Z_BUTTON) {
			lego_spin_degrees(LEFT, SMALL_SPIN_ADJUST, 10);
			adjustment -= SMALL_DRIVE_ADJUST;
		}
	}
