...
void go_to_pom_pile() {
	// FIXME: Should be based on DISTANCE or use CAMERA.
...
void sort_while_moving() { // FIXME: Add DISTANCE parameter.
	...
	for (i = 0; i < 7; i++) {
		set_servo_position(CLAW_SERVO, CLAW_HALF_OPEN_POSITION);
		move_servo_at_speed(CLAW_SERVO, CLAW_CLOSED_POSITION, SLOW_SERVO_SPEED);
	} 
	msleep(2000);  // FIXME.
	pause();
}
...
void go_close_to_first_pom_pile() {
	...
	// May need to "wiggle".
	lego_drive_distance(4.5, TRAVELLING_SPEED, FORWARDS); // 3 inches
	pause();
}
